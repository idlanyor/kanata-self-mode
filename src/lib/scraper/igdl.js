...
export const igDl = async (url) => {
    try {
        ...
        const data = await page.evaluate(() => {
            const items = document.querySelectorAll("#search-result .download-items");
        ...
        if (Array.isArray(data)) {
            return {
                ...
                data: data
                    .filter(item => item.thumbnail !== 'https://igsaved.app/imgs/loader.gif')
                    .map(item => ({
        ...
        return {
            status: true,
            message: "Success scraping Instagram content",
            data: data.thumbnail === 'https://igsaved.app/imgs/loader.gif' ? null : {
...
// (async () => {
//     const testUrl = 'https://www.instagram.com/jkt48feni/p/DFr5cYbTGLe/?img_index=1'
//     const result = await igDl(testUrl)
//     console.log(result)
